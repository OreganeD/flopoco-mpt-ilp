...
namespace flopoco {
    ...
    MultiplierTileCollection::MultiplierTileCollection(Target *target, BaseMultiplierCollection *bmc, int mult_wX, int mult_wY, bool superTile, bool use2xk, bool useirregular, bool useLUT, bool useDSP, bool useKaratsuba, bool squarer) {
        ...
        if(useDSP) {
            ...
            addBaseTile(target, new BaseMultiplierDSP(17, 24, 1));
        }

        if(useLUT) {
            addBaseTile(target, new BaseMultiplierLUT(3, 3));
            addBaseTile(target, new BaseMultiplierLUT(2, 3));
            addBaseTile(target, new BaseMultiplierLUT(3, 2));
            addBaseTile(target, new BaseMultiplierLUT(1, 2));
            addBaseTile(target, new BaseMultiplierLUT(2, 1));
            addBaseTile(target, new BaseMultiplierLUT(1, 1));
        }

        if(superTile){
            for(int i = 1; i <= 12; i++) {
                addSuperTile(target, new BaseMultiplierDSPSuperTilesXilinx((BaseMultiplierDSPSuperTilesXilinx::TILE_SHAPE) i));
            }
        }
        ...
        if(use2xk){
            ...
            for(int x = variableTileOffset; x <= mult_wX; x++) {
                addVariableXTile(target, new BaseMultiplierXilinx2xk(x, 2));
            }

            for(int y = variableTileOffset; y <= mult_wY; y++) {
                addVariableYTile(target, new BaseMultiplierXilinx2xk(2, y));
            }
        }

        if(useirregular){
            for(int i = 1; i <= 8; i++) {
                addBaseTile(target, new BaseMultiplierIrregularLUTXilinx((BaseMultiplierIrregularLUTXilinx::TILE_SHAPE) i));
            }
        }
...
//        cout << MultTileCollection.size() << endl;
    ...
    void  MultiplierTileCollection::addBaseTile(Target *target, BaseMultiplierCategory *mult) {
        mult->setTarget(target);
        MultTileCollection.push_back(mult);
        BaseTileCollection.push_back(mult);
    }

    void  MultiplierTileCollection::addSuperTile(Target *target,BaseMultiplierCategory *mult) {
        mult->setTarget(target);
        MultTileCollection.push_back(mult);
        SuperTileCollection.push_back(mult);
    }

    void  MultiplierTileCollection::addVariableXTile(Target *target,BaseMultiplierCategory *mult) {
        mult->setTarget(target);
        MultTileCollection.push_back(mult);
        VariableXTileCollection.push_back(mult);
    }

    void  MultiplierTileCollection::addVariableYTile(Target *target,BaseMultiplierCategory *mult) {
        mult->setTarget(target);
        MultTileCollection.push_back(mult);
