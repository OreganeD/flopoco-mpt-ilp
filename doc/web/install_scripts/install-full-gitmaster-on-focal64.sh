...
FLOPOCO_INSTALL_DIR=$PWD


cd $FLOPOCO_INSTALL_DIR
#WCPG
...
#SCIP_DIR=$FLOPOCO_INSTALL_DIR/scipoptsuite-6.0.0 # modify just this
...
cd flopoco && mkdir build && cd build && cmake  -DWCPG_PREFIX_DIR="$WCPG_PREFIX_DIR"  -DSCALP_PREFIX_DIR="$SCALP_PREFIX_DIR" -DPAG_PREFIX_DIR="$FLOPOCO_INSTALL_DIR/pagsuite" .. && make -j4 &&  cd $FLOPOCO_INSTALL_DIR
